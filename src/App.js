...
import AddNewMusic from "./containers/AddNewMusic/AddNewMusic";
...
class App extends Component {
    ...
    handleChangedOwned = (id) => {
        let albums = [...this.state.albumData];

        let albumToUpdate = albums.findIndex((album) => {
            return album.id === id;
        });

        if (albumToUpdate > -1) {
            let album = albums[albumToUpdate];
            album.owned = !album.owned;

            albums[albumToUpdate] = album;
    ...
    handleRemoveAlbum = (id) => {
        ...
        let albumToRemove = albums.findIndex((album) => {
            return album.id === id;
        });

        if (albumToRemove > -1) {
            albums.splice(albumToRemove, 1);
    ...
    render() {
        return (
            <Layout>
                <MusicLibrary
                    artists={this.state.artistData}
                    albums={this.state.albumData}
                    handleChangedOwned={this.handleChangedOwned}
                    handleRemoveAlbum={this.handleRemoveAlbum}
                />
                <AddNewMusic />
            </Layout>
            // <div className="App" >

...
const InputSelect = (props) => {
        ...
        if (userInput === "") {
            ...
            let filteredData = props.data.map((data) => {
                if (
                    ...
                    return (
                        <div
                            className="album-dropdown__item"
                            key={data.id}
                            onClick={props.onUpdate}
                            data-value={data[props.dataTitle]}
                        >
                            {data[props.dataTitle]}
                        </div>
                    );
    ...
    return (
        <div className="field">
            <label htmlFor={"album-" + props.dataTitle}>
                {props.dataTitle}:
            </label>
            <div className="control">
                <input
                    type="text"
                    name={"album-" + props.dataTitle}
                    id={"album-" + props.dataTitle}
                    className="input album__input album__input--text"
                    onBlur={onBlur}
                    onChange={onChange}
                    value={props.newData}
                    autoComplete="off"
            ...
            <div className="album-dropdown">
                {inputState.filteredData.length ? (
                    <div>{inputState.filteredData}</div>
                ) : null}
            </div>
        </div>
    );
